...
#be passed as arguments
    ...
    while abs(f(x0)) > epsilon:#Fill in the while loop condition for our stoping criteria:
        x = x0 - f(x0)/df(x0) #newtons update step
        x0 = x # set x0 = x for the next step in 
        #newtons method
        num_steps = num_steps + 1 #update the number 
        #of steps
        print("the ",num_steps," iteration is ", x0)
    return x,num_steps

f = lambda x: x**3 -9*x + 2# Fill in the function
# we want to use Newton's method on to find a root

df = lambda x: 3*x**2 - 1#Fill in the derivative of 
# the funciton we want to find a root 

epsilon = 10**-8

x0 = 4 #our initial guess
